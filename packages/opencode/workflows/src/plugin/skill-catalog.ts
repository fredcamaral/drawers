 ...
 * this is the plugin layer, not the runtime layer. When NEITHER variable is set,
 * returns undefined (the user root is skipped) instead of fabricating the
 * literal `"undefined/.config/..."` path and silently scanning nothing.
 */
function resolveUserConfigDir(): string | undefined {
	const xdg = process.env.XDG_CONFIG_HOME;
	if (xdg !== undefined && xdg.length > 0) {
		return joinPath(xdg, "opencode/skill");
	}
	const home = process.env.HOME;
	if (home !== undefined && home.length > 0) {
		return joinPath(`${home}/.config`, "opencode/skill");
	}
	return undefined;
}
