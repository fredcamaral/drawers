 ...
 *
 * The terminal-notice path (core's `createChatMessageHook`) injects TWO parts on the
 * parent's next message: a one-line VISIBLE toast (`✅ wf_x 'name' completed in 32s`)
 * and a model-only SYNTHETIC part carrying `notice.hint`. Historically that hint was a
 * bare pointer (`workflow_status run_id=… for the result`) — a doorbell, not content.
 * This module renders the hint into a real digest: a header (status, duration, agent
 * tally, total tokens) followed by every agent grouped by phase, each with its stats,
 * the CONCLUSION it passed forward, and its degrade note when it failed.
 *
