...
describe("createTaskStore", () => {
	...
	test("default baseDir honors PI_DRAWERS_DATA_DIR (now folded into resolveDataBaseDir)", async () => {
		const prevEnv = process.env.PI_DRAWERS_DATA_DIR;
		const envDir = await mkdtemp(join(tmpdir(), "env-"));
		process.env.PI_DRAWERS_DATA_DIR = envDir;
		try {
			...
			if (prevEnv === undefined) {
				delete process.env.PI_DRAWERS_DATA_DIR;
			} else {
				process.env.PI_DRAWERS_DATA_DIR = prevEnv;
			}
		}
	});
	...
	test("default baseDir honors XDG_DATA_HOME", async () => {
		...
		try {
			...
			await store.dispose();
			const expectedDir = join(xdg, "pi-drawers", "tasks");
			const files = await readdir(expectedDir);
...
describe("resolveDataBaseDir", () => {
	...
	beforeEach(() => {
		prevEnv = process.env.PI_DRAWERS_DATA_DIR;
	...
	test("explicit wins over env and XDG", () => {
		process.env.PI_DRAWERS_DATA_DIR = "/env/base";
		process.env.XDG_DATA_HOME = "/xdg";
	...
	test("XDG fallback when env is unset, namespaced under pi-drawers", () => {
		delete process.env.PI_DRAWERS_DATA_DIR;
		process.env.XDG_DATA_HOME = "/xdg";
		expect(resolveDataBaseDir()).toBe(join("/xdg", "pi-drawers"));
	});
	...
	test("home fallback when neither env nor XDG is set", () => {
		...
		expect(resolveDataBaseDir()).toBe(
			join(homedir(), ".local", "share", "pi-drawers"),
		);
	});

	test("empty-string env is ignored, falling through to XDG", () => {
		process.env.PI_DRAWERS_DATA_DIR = "";
		process.env.XDG_DATA_HOME = "/xdg";
		expect(resolveDataBaseDir()).toBe(join("/xdg", "pi-drawers"));
	});
...
describe("isValidTask — validation arms", () => {
	...
	test.each([
		...
		["non-string appendSystemPrompt", { appendSystemPrompt: 42 }],
		["non-array agentTools", { agentTools: "read,grep" }],
		["agentTools with non-string element", { agentTools: ["read", 7] }],
	] as Array<
		[string, Record<string, unknown>]
	...
	test("pi-native knobs validate when well-typed and may be absent", () => {
		expect(
			isValidTask(
				raw({
		...
		expect(isValidTask(raw({ agentTools: [] }))).toBe(true);
		const t = raw();
		delete t.appendSystemPrompt;
		delete t.agentTools;
