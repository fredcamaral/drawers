 ...
 * `.quiet()` to the ShellPromise (the plugin host shares fd 1/2 with the host
 * renderer — an un-quieted command's stdout/stderr would punch raw bytes through
 * the TUI alt-buffer and corrupt the screen), and is inspected by `exitCode` — a
 * non-zero git never rejects into the run. A non-repo (bare/detached/zero-commit-
 * safe) is detected ONCE by `ready()`, which latches the checkpointer dead with a
 * single warn; every later call is a silent no-op. When no `shell` is injected the
 * whole subsystem is a documented no-op. This mirrors the feed writer's dead-state
 * latch. The module imports nothing from engine.ts — the dependency is one-way
 * (engine constructs this), matching git-deny.ts precedent.
 *
 * The host shell is a TAGGED-TEMPLATE callable (`$\`git status\``), NOT an argv API:
 ...
 * output carries `.exitCode` and a SYNCHRONOUS `.text()` — this module reads
 * `.text()` synchronously off the awaited output.
 */

/**
 * The host shell primitive — a Bun-shell-shaped tagged-template runner the engine
 * wires to a Node-runnable implementation (W2 seam). Defined structurally here so
 * the module carries no opencode coupling: the engine supplies any value matching
 * this contract (`cwd(dir).nothrow()` → tagged-template call → `.quiet()` → an
 * awaitable carrying `.exitCode` and a synchronous `.text()`).
 */
...
/** The awaitable a tagged-template invocation returns, carrying `.quiet()`. */
export type ShellPromise = Promise<ShellOutput> & {
	quiet(): ShellPromise;
};

/** A repo-bound, fenced tagged-template shell namespace. */
export type ShellNamespace = (
	strings: TemplateStringsArray,
	...expressions: unknown[]
) => ShellPromise;

/**
 * The host shell, chainable like Bun's `$`: `cwd(dir).nothrow()` yields a
 * tagged-template namespace. The engine wires a Node-runnable implementation
 * (W2 seam — there is no host `$` under Node/pi); `undefined` makes the whole
 * checkpointer a no-op.
 */
export type BunShell = {
	cwd(directory: string): BunShell;
	nothrow(): BunShell;
} & ShellNamespace;
...
export interface CreateGitCheckpointerOptions {
	/** The host shell; `undefined` makes the whole checkpointer a no-op. */
...
/** The text view of an awaited shell output's stdout (sync `.text()`). */
...
export function createGitCheckpointer(
	 ...
	 * plugin host runs in the same OS process as the host renderer and shares fd 1/2.
	 * The default shell ECHOES each command's stdout/stderr to those descriptors (only
	 * the lazy `.text()` auto-quiets — but this module awaits first, then reads `.text()`
	 * off the resolved buffer, which does NOT). Without `.quiet()`, git's commit summary
	 * ("[branch sha] workflow checkpoint: …") punches raw bytes through the TUI
	 * alt-buffer and corrupts the screen. `.quiet()` still buffers, so the
	 * `.exitCode`/`readText()` reads downstream are unchanged — it only suppresses the
	 * echo. The {@link createGitCheckpointer} output-suppression test guards this.
	 */
	...
	async function checkpoint(meta: CheckpointMeta): Promise<CheckpointResult> {
		...
		// too, not just worktree-dirty paths. The shell escapes the interpolated array
