 ...
 * (here), exactly as the pi subagent example and the background-agents extension
 * do: an agent is a markdown file with optional YAML frontmatter (`model`,
 * `tools`) and a body that becomes the appended system prompt. We look the name up
 * in pi's standard agent locations:
 ...
 * Project wins over user when both define the same name.
 *
 * The resolved knobs are threaded into the runner's `LaunchRequest` as
 * `appendSystemPrompt`/`tools`/`model` — the `agent()` → `LaunchRequest` seam the
 * workflows engine wires through `resolveAgentKnobs` (since pi has no `--agent`).
 * If the name does not resolve to a file we return `undefined` and the child runs
 * pi's DEFAULT coding assistant — resolution failure is NOT a launch failure.
 *
 * This is the same resolver the background-agents extension ships; it is duplicated
 * here (rather than cross-imported) so the two extensions stay independently
 * installable, and threaded into `createWorkflowEngine` from the factory.
 *
