 ...
 * `shell.cwd(dir).nothrow()` + `.quiet()` (the host shares fd 1/2 with the renderer
 * — an un-quieted command corrupts the TUI; the reason is documented at
 * `git-checkpoint.ts:215-228`) and is inspected by `.exitCode`. A non-zero git
 * never rejects into the run. With NO shell (or a non-git checkout) the classifier
 * never fabricates a git verdict: it returns `untracked`/`missing` from the injected
 * `exists` flag alone — emptiness/ignored-ness is UNPROVABLE without git, parity
 * with the checkpointer's `available:false` honesty.
 */
