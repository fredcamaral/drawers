 ...
 * what jiti loads from). `import.meta.resolve` returns that real on-disk path even
 * through bun's isolated store, and `cli.js` sits beside `index.js`. This is
 * exactly the recipe the proven core smoke uses.
 *
...
const CLI_OVERRIDE_ENV = "PI_DRAWER_WORKFLOWS_CLI";
 ...
 *
 *   1. `$PI_DRAWER_WORKFLOWS_CLI` — explicit operator override.
 ...
 *
 * THROWS if none yields an existing file — a workflows extension with no spawnable
 * pi entry cannot launch any child agent, so failing loud at `session_start` beats
 * every `workflow` run silently erroring later. The message names the override env
...
export function resolvePiCliPath(): string {
	...
	const tried = candidates
		...
		.join(", ");
	throw new Error(
		`pi-drawer-workflows: could not resolve a spawnable pi CLI (dist/cli.js). ` +
