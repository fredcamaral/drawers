...
import { createCadenceStore, type Directive, isValidDirective } from "./store";
...
describe("isValidDirective", () => {
	test("accepts a well-formed loop and a goal-without-interval", () => {
		expect(isValidDirective(sample)).toBe(true);
		expect(
			isValidDirective({
				id: "g1",
				sessionID: "s1",
	...
	test("rejects non-objects, bad discriminators, and broken numerics", () => {
		expect(isValidDirective(null)).toBe(false);
		expect(isValidDirective("nope")).toBe(false);
		// Unknown kind / status.
		expect(isValidDirective({ ...sample, kind: "cron" })).toBe(false);
		expect(isValidDirective({ ...sample, status: "paused" })).toBe(false);
		// Empty id.
		expect(isValidDirective({ ...sample, id: "" })).toBe(false);
		// NaN/zero numerics the cap depends on.
		expect(isValidDirective({ ...sample, iterations: Number.NaN })).toBe(false);
		expect(isValidDirective({ ...sample, maxIterations: 0 })).toBe(false);
		// A loop with no positive interval.
		expect(isValidDirective({ ...sample, intervalMs: undefined })).toBe(false);
	});
});
