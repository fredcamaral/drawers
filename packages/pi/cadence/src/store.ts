 ...
 *
 * Persistence model: an injectable {@link FsFacade} (default `nodeFsFacade()` from
 * `@drawers/pi-core`), one JSON file per directive under `<dataDir>/cadence/`,
 * written via a UNIQUE tmp file (`<target>.<n>.tmp`) then atomically renamed over
 * the target. The unique tmp suffix means two concurrent writes for the same id
 * never share a tmp path and so never tear each other; the rename is the atomic
 * publish (last writer wins). The store itself does NOT serialize same-id writes —
 * the engine's per-directive in-flight guard already guarantees at most one write
 * per id in flight, so a serialization queue would be redundant.
 *
 ...
 *
 * Port note: this is the opencode store with `@drawers/core` swapped for
 * `@drawers/pi-core`. pi-core's generic `createTaskStore<T>` is intentionally NOT
 * reused — its `T extends StoredRecord` constraint requires `status: TaskStatus`
 * (`pending|running|completed|error|cancelled`), which conflicts with the cadence
 * `active|done|stopped` union. Cadence only ever persists `active` directives
 * (terminal ones are `delete()`d), so the generic store's TTL sweep and status
 * machinery buy nothing here; the directive shape stays decoupled from `TaskStatus`.
 */
...
export interface CadenceStoreOptions {
	...
	baseDir?: string;
	/** Injectable fs facade; defaults to `nodeFsFacade()`. */
	fs?: FsFacade;
...
export function isValidDirective(value: unknown): value is Directive {
