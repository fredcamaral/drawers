...
| 8 | Live observability + native viewer: real-time per-run feed with per-agent tokens/tool-calls/duration, external cancel channel, and a `./tui`-surface full-screen viewer (Phases \| Agents \| Detail) mirroring CC's `/workflows` overlay | 8.1, 8.2, 8.3 | Complete |
...
**Deviations:**
- **8.1.1:** One pre-existing test (`progress events > start and end events carry label and phase`) asserted the exact agent:end shape via toEqual and had to be updated to include the now-present sessionID — this is the expected consequence of the additive field, not a divergence from the task. Two of the six new tests passed on first RED run because they assert the negative (cached/launch-throw paths emit no agent:launched and no sessionID); these guard the "untouched paths" requirement.
- **8.1.2:** None of substance. Plan-doc line numbers had drifted (settle sites were not literally at 673/690/707); I located the actual `settleRecord` call sites by structure and wired all three. Stored the per-run FeedWriter in a module-internal `feeds: Map<runId, FeedWriter>` rather than on the exported RunHandle, to keep the feed type off the public surface — stopRun reaches the writer through it for the cancel run:end line. Both are within the task's intent (one writer per run, run:end at each settle site).
- **8.1.3:** Used Parameters<SessionRunner["handleEvent"]>[0] to type the collector's Event rather than importing from @opencode-ai/sdk: the vision's wording implied an SDK import, but the workflows package has no direct @opencode-ai/sdk dependency and tsc rejected it (the engine already derives the event type this way). Behavior is identical; dependency surface unchanged. The vision also says unregister happens "after the final enrichment on agent:end (Task 8.1.4 consumes the snapshot)" — since 8.1.4 is not yet implemented, 8.1.3 unregisters on the choke-point agent:end sighting; 8.1.4 will move the unregister to after its enrichment.
- **8.1.4:** Cancel-test premise corrected during GREEN: aborting a run settles the in-flight second agent on a non-completed terminal status, so its agent:end DOES fire (status "cancelled", with a status_cancelled note). The implementation correctly rolls up BOTH agents on cancel (the completed first + the aborted second), which is strictly more faithful to "partial agents persist" than my initial assumption that the in-flight agent would leave no end. Test was rewritten to assert the real, correct behavior (2 entries). No implementation change resulted from this.
- **8.1.5:** Per the binding vision ("Rework the render"), the flat-render tests from Tasks 4.1.3/6.2.1 were migrated to the new CC-style format. Notably, per-agent duration now comes from the engine-stamped enriched durationMs field (Task 8.1.4) rendered via the new spaced formatDuration, not from in-tool start/end pairing with humanizeDuration; the three affected 6.2.1 tests were updated to assert the enriched-durationMs contract. The done/total counter counts every terminal occurrence (cached included), since cached is a finished replay.
- **8.2.1:** The vision named both `SUBDIR_CONTROL` AND a `controlDir = subdir(SUBDIR_CONTROL)` resolution "beside feedDir". An unused `controlDir` local trips biome's recommended `noUnusedVariables` rule (the clean baseline is warning-free), and there is no honest non-speculative consumer until Task 8.2.2. I dropped the dangling `controlDir` derivation (8.2.2 adds it one line above where it threads it into the watcher) and kept `SUBDIR_CONTROL` as the durable "layout in one place" artifact, annotated with an established-convention biome-ignore naming 8.2.2 as the consumer. Net: the vision's intent (layout vocabulary declared beside siblings, run:cancel-requested as a first-class FeedEvent) is honored, the lint gate stays green, and no speculative export was introduced. The `controlDir` path resolution is deferred to 8.2.2 at its point of use.
- **8.2.2:** RED-first was captured authentically only for the control.ts unit (module-not-found failure shown in redProof). The engine-level e2e test (a sentinel cancels a live run) was added after the engine wiring, so it did not have an isolated pre-wiring RED capture; its truth is covered by the unit RED plus the green e2e. No divergence from the task's Implementation vision. Note: the live-cancel engine e2e uses the makeCompletingClient two-sequential-agent pattern (mirroring the existing in-flight cancel feed test) rather than HANGING+inert client, because the inert client never drains the detached settle branch and so never writes the terminal run:end the test must assert.
- **8.2.3:** Two minor faithful corrections to the Implementation vision's stated facts, no contract change: (1) the vision claims `mkdir`/`writeFile` from node:fs/promises are "already imported (run-smoke.ts:57)" — they were not (line 57 imported only mkdtemp/readdir/readFile/rm), so I added `mkdir` and `writeFile` to that import (biome then wrapped it multi-line to satisfy line width). (2) The smoke harness lives outside every named typecheck project (workflows tsconfig includes only `src`), so the honest TDD RED was captured by type-checking the harness via a root-anchored temp tsconfig extending tsconfig.base.json, then removed. The live `bun run smoke:workflows` (A–F against a real headless opencode) was NOT executed in the unit phase: it requires live model auth, network, and real token spend across multiple minutes — a credentialed/interactive verification, not a sandbox gate. Structural correctness (compiles strict, lints clean, wired into main, exact sentinel path/name matching 8.2.2's watcher) is fully verified; the live run is recorded in the smoke evidence below.
- **8.3.1:** Initially added the `summarize` helper + its two tests preemptively, but the plan scopes `summarize` to Task 8.3.4 (it appears in 8.3.4's files-to-modify and verification, not 8.3.1). Removed it before committing to keep this commit atomic to 8.3.1's contract. Also added `phaseMarker` to the shared format.ts (not named explicitly in the helper list but required for the reducer's per-phase marker derivation, which the vision mandates "reuse the SAME glyph table as statusMarker"); workflow-status.ts now imports it too, removing its local duplicate — consistent with the one-source-of-truth intent.
- **8.3.2:** Two minor, vision-respecting calls: (1) exposed a tick() method on the returned tailer (not named in the vision's { start; stop } shape) as the deterministic per-read test seam, mirroring the established control.ts tick() idiom the Verification block requires for driving readNew() against injected fs; the in-flight latch and offset behavior are unchanged. (2) The readFn/statFn defaults split node:fs/promises stat + open/read/close (read range) from node:fs watch — the vision said "node:fs's watch/open/read", which this matches exactly; stat is the size source the vision mandates ("stat the file and read only [offset, size)").
- **8.3.3:** In-route key handling: the plan's vision named opencode-internal `useBindings` (`@tui/keymap`), which is not exported to external plugins. Used the portable `api.keymap.registerLayer({ commands, bindings })` instead — the documented external-plugin equivalent. Added `solid-js@1.9.12` as an optional peer+dev dependency (not in the literal Files list): it is a hard runtime+typecheck requirement for any solid `.tsx`, provided by the host as an opentui peer, pinned to the version `@opentui/solid@0.3.2` declares. The route component is lazily imported (solid `lazy`+`createComponent`) so the JSX-free `.ts` entry keeps the smoke test's no-opentui-runtime contract; the test asserts module shape + registration + sentinel-write only, never mounting JSX.
- **8.3.4:** One justified deviation: biome's lint/a11y/noStaticElementInteractions flags `onMouseDown` on the opentui `text`/`box` custom elements (it treats unknown lowercase tags as static DOM nodes). These are terminal-renderer elements with no accessibility tree — a false positive the reference plugins (todo.tsx/diff-viewer.tsx) never hit because they live outside this repo's linting. Suppressed with a precise inline `biome-ignore` (the repo's sanctioned escape hatch, already used in index.test.ts) rather than weakening the global config. `summarize` takes an explicit `now` arg (not an internal clock) to keep the reducer module clock-free and the unit test deterministic, matching the plan's clock-free contract; the sidebar passes Date.now().

**Smoke evidence (live A–F against a real headless opencode, opencode 1.16.2, model opencode/claude-haiku-4-5):**
- **A:** review workflow (pipeline + parallel + phase + sub-workflow) — PASS (run wf_05pium5j completed; result carried "reviewed fs.ts" / "reviewed net.ts" / "helper-marker").
- **B:** workflow_stop in-session cancel — PASS (run wf_duivi3v3 settled "cancelled").
- **C:** resume in second process, all-cached, no child relaunch — PASS (resumed run wf_oxoevloc completed; child-task store unchanged 6 to 6; returnValue reproduced from cache).
- **D:** structured output agent({schema}) e2e — PASS (run wf_2hgyb7ii returnValue={"r":{"verdict":"42 is even"}}, validated object with string verdict).
- **E:** liveness veto over-block guard (~15s quiet long-tool turn completes whole) — PASS (run wf_xgpjoepj returnValue carried LIVENESS_FINAL_E2E_77).
- **F:** Epic 8.2 external filesystem sentinel cancel of a LIVE run — PASS (run wf_vx3hr7sq captured running, harness touched workflow-control/wf_vx3hr7sq.cancel, engine poll loop observed it inside the live server, settled "cancelled", consumed the sentinel).
- Harness exit code 0; final summary printed "================ PASS ================". No retries needed — every scenario succeeded on attempt 1/3.
...
**Mechanism decision (pinned).** A **poll loop**, not `fs.watch`. Three reasons, decisive: (1) the injected `FsFacade` (`packages/core/src/persistence.ts:38-53`) exposes `readdir`/`rm({force:true})` but NO `watch` — a watch would break the in-memory test fs and the engine's fs-injection contract; (2) there is no existing periodic loop in the engine to piggyback — the progress choke (`engine.ts:825`) and `handleEvent` (`engine.ts:1052`) are event-driven, so a quiet run (slow child, no SDK events) would never tick a piggybacked check; (3) sentinel detection is exactly `readdir(controlDir)` + `rm(sentinelPath)`, both already in the facade. One `setInterval` per engine instance (default 1000ms), with `setIntervalFn`/`clearIntervalFn` injectable for deterministic tests (the seam `workflow-status.ts:66` already establishes). The loop is the cheapest mechanism that fits the existing injection surface.
...
**Implementation vision:** Add `RunCancelRequestedLine { type: "run:cancel-requested"; runId: string; at: number }` to `feed.ts`, doc-commented as "emitted once when an external sentinel requests cancel, BEFORE the terminal `run:end`; a viewer renders a 'cancelling…' state on sight", and include it in the `FeedEvent` union. Add `const SUBDIR_CONTROL = "workflow-control";` to `engine.ts:319` (immediately after `SUBDIR_FEED`) and resolve `const controlDir = subdir(SUBDIR_CONTROL);` beside `feedDir` (`engine.ts:470`). No behavior change in this task — this is the vocabulary + path layer the watch loop (8.2.2) consumes. Do NOT widen the runtime `ProgressEvent` union: this is a feed-only/engine-only line, exactly like `RunStartLine`/`AgentStatsLine`.

**Files:**
- Modify: `packages/workflows/src/plugin/feed.ts` (new line type + union member)
- Modify: `packages/workflows/src/plugin/engine.ts` (`SUBDIR_CONTROL` constant + `controlDir` resolution)
- Test: `packages/workflows/src/plugin/feed.test.ts` (a `run:cancel-requested` line round-trips through `append` and parses back as JSON with the discriminant)

**Verification:** `bun test packages/workflows/src/plugin/feed.test.ts` — RED-first: an appended `run:cancel-requested` line is the verbatim JSON the writer wrote, ordered in the chain like any other line. `bunx tsc --noEmit -p packages/workflows` — the union widening type-checks and no existing `FeedEvent` consumer (the writer's `JSON.stringify`, any switch) breaks. `biome check packages/workflows/src/plugin/feed.ts`.

**Done when:** `run:cancel-requested` is a first-class `FeedEvent` member that round-trips through the writer, and `SUBDIR_CONTROL`/`controlDir` exist beside the other subdir constants — with zero behavioral change.
...
**Implementation vision:** New module `packages/workflows/src/plugin/control.ts`: `createControlWatcher({ dir, fs, intervalMs, setIntervalFn, clearIntervalFn, onCancel, logger })` → `{ start(): void; stop(): void; tick(): Promise<void> }`. `tick()` is the unit of work, exposed so tests drive it directly without timers: `readdir(dir)` (catch ENOENT/any error → treat as empty, log once at debug — a missing control dir is the normal steady state); filter to names ending `.cancel`; for each, derive `runId = name.slice(0, -".cancel".length)` and call `await onCancel(runId)`; then `rm(join(dir, name), { force: true })` to consume the sentinel (always consume, even when the run is unknown/terminal — a stale sentinel must not accumulate or re-fire). `rm` failures are logged once and swallowed (consume-and-forget; a sentinel that can't be removed will retry next tick, which is harmless because `onCancel` is idempotent). `start()` arms `setIntervalFn(() => { void tick(); }, intervalMs)` exactly once (guard double-start); `stop()` clears it. Engine wiring in `createWorkflowEngine`: construct `onCancel = (runId) => { const handle = runs.get(runId); if (handle?.record.status === "running") { feeds.get(runId)?.append({ type: "run:cancel-requested", runId, at: clock.now() }); } stopRun(runId); }` — the `run:cancel-requested` line is appended ONLY for a live run (so a viewer sees the cancelling state) and BEFORE `stopRun`, but `stopRun` itself remains the single cancel authority and is safe to call unconditionally (its own `status !== "running"` guard makes unknown/terminal ids no-ops). Construct the watcher with `controlDir` (8.2.1), the engine `fs` adapted to the readdir/rm subset, `intervalMs: opts.controlPollMs ?? 1000`, and the injectable interval fns (default `globalThis.setInterval`/`clearInterval`); call `watcher.start()` after `runs` and `feeds` exist (near `engine.ts:557`) and `watcher.stop()` first thing in `dispose()` (`engine.ts:1144`). Add `controlPollMs?: number` and the interval-fn injection to `CreateWorkflowEngineOptions` (`engine.ts:240-259`), defaulted, so tests pin the cadence and drive ticks. The watcher reuses the engine `FsFacade` (no new fs surface): `readdir(dir)` and `rm(path, {force:true})` are both already on `FsFacade`.

**Files:**
- Create: `packages/workflows/src/plugin/control.ts`
- Modify: `packages/workflows/src/plugin/engine.ts` (watcher construction + `onCancel`, `start()` after the maps exist, `stop()` in `dispose`, options widening)
- Test: `packages/workflows/src/plugin/control.test.ts` (watcher unit tests against an in-memory fs), `packages/workflows/src/plugin/engine.test.ts` (engine-level: a sentinel cancels a live run)

**Verification:** `bun test packages/workflows/src/plugin/control.test.ts` — RED-first: (1) `tick()` on a dir containing `wf_abc.cancel` calls `onCancel("wf_abc")` once and removes the file; (2) a missing/ENOENT dir yields no calls and no throw; (3) a sentinel with no `.cancel` suffix is ignored; (4) `start()` arms exactly one interval (double-`start` is a no-op), `stop()` clears it; (5) `onCancel` rejection/`rm` failure is swallowed (the loop survives). `bun test packages/workflows/src/plugin/engine.test.ts` — engine e2e against `makeFs`: a live `HANGING` run, then write `<controlDir>/<runId>.cancel` into the facade and `await engine` control `tick()` (via the injected seam) → the record settles `cancelled`, the run's feed contains a `run:cancel-requested` line BEFORE the terminal `run:end`, and the sentinel file is gone from the facade; a sentinel for an UNKNOWN runId is consumed (removed) with no record change. `bunx tsc --noEmit -p packages/workflows` + `biome check packages/workflows/src/plugin/control.ts`.

**Done when:** writing `<dataDir>/workflow-control/<runId>.cancel` causes the engine, on its next poll, to cancel that live run through the existing `stopRun` path (children aborted, record `cancelled`, `run:cancel-requested` then `run:end` in the feed) and unlink the sentinel; stale/unknown sentinels are consumed harmlessly; the loop stops cleanly on `dispose()`.
...
**Implementation vision:** Add `scenarioF(dataDir, beforeCount)` exercising the external sentinel. Launch the `longrun` workflow WITHOUT any in-session stop (prompt: call `workflow` only, reply with the run_id; do NOT call `workflow_stop`), via `runUntilRunAppears`. Snapshot prior ids (like Scenario D/E, `run-smoke.ts:429`) and identify the NEW run by polling `readRuns` until a record with status `running` and an id not in the prior set appears (a short bounded poll, ~10s, reusing the `sleep`/deadline shape of `waitForTerminal`); capture its `runId`. Then, from the HARNESS process (the external actor), `mkdir(join(dataDir, "workflow-control"), { recursive: true })` and `writeFile(join(dataDir, "workflow-control", runId + ".cancel"), "")` using `node:fs/promises` (already imported, `run-smoke.ts:57`) — the literal `touch`. Assert with `waitForTerminal(dataDir, r => r.id === runId && r.status === "cancelled", 30_000)`: the engine's poll loop (≤1s cadence) must observe the sentinel and cancel the live run. Then assert the sentinel was CONSUMED: `readdir(join(dataDir, "workflow-control"))` no longer contains `runId + ".cancel"` (poll briefly — the unlink happens in the same tick as the cancel but the file probe races the settle; a ~5s bounded poll covers it). Wire `scenarioF` into `main()` after Scenario E (`run-smoke.ts:592`), threading `afterE` run-count, and add its PASS line to the summary block (`run-smoke.ts:594-601`). Keep the harness's existing structure: authoritative on persisted state, retry on model refusal via `runUntilRunAppears`, no assertions on model stdout.

**Files:**
- Modify: `packages/workflows/test-harness/run-smoke.ts` (new `scenarioF` + `main()` wiring + summary line)

**Verification:** `bun run smoke:workflows` (root script, `package.json`) with `OPENCODE_BIN`/`SMOKE_MODEL` set — the full harness runs A–F and prints PASS; Scenario F specifically logs the captured live runId, the external `touch`, the `cancelled` settle, and the consumed-sentinel confirmation. The unit/typed gates from 8.2.1–8.2.2 stay green: `bun test` (repo root), `bun run typecheck`, `bun run lint`.

**Done when:** the smoke harness proves, against a real headless opencode, that an external filesystem `touch <dataDir>/workflow-control/<runId>.cancel` cancels a live (in-flight, not in-session-stopped) run — settling it `cancelled` and consuming the sentinel — and the full A–F suite passes.
...
**Implementation vision:** New module `packages/workflows/src/tui/reducer.ts` (the `./tui` surface's pure core, no opentui/solid imports — unit-testable under plain `bun test`). Two pieces:
1. `parseFeedLine(line: string): FeedEvent | undefined` — `JSON.parse` inside try/catch (a truncated trailing line during a live tail returns `undefined`, never throws), then a discriminant check on `type` against the known `FeedEvent` members; unknown/partial objects return `undefined`. Re-export `FeedEvent` from `feed.ts` rather than redefining it (one source of truth for the wire shape).
2. `createRunStateReducer()` → `{ apply(event: FeedEvent): void; state(): RunViewState }`, fed lines in file order. `RunViewState` = `{ runId?: string; status: "running" | "cancelling" | RunStatus; startedAt?: number; endedAt?: number; phases: PhaseView[] }`, where `PhaseView = { name: string; done: number; total: number; marker: string; agents: AgentView[] }` and `AgentView = { label; phase?; sessionID?; status?; model?; tokens?: number; toolCalls?; lastTools?: string[]; durationMs?; note?; startedAt?; updatedAt? }`. Reduction rules, mirroring `workflow-status.ts` pairing exactly so live and server views agree: `run:start` sets `runId`/`startedAt`; `agent:start` opens a new agent occurrence in its phase group (FIFO, preserving start order — a still-running occurrence has `status: undefined`); `agent:launched` binds `sessionID`/`model`/`agentType` onto the most-recent unlaunched same-label occurrence; `agent:stats` updates the matching occurrence's live `tokens` (summed via the same total formula as `totalTokens`, `workflow-status.ts:167`)/`toolCalls`/`lastTools`/`updatedAt` BY sessionID; `agent:end` closes the oldest open same-label (or same-sessionID, when present) occurrence with `status`/`durationMs`/`tokens`/`toolCalls`/`note` from the enriched end, recomputes the phase `done`/`total`/`marker`; `run:cancel-requested` flips top-level `status` to `cancelling`; `run:end` sets `status`/`endedAt`. Phase `(no phase)` bucket and the `marker` derivation reuse the SAME glyph table as `statusMarker` (`workflow-status.ts:191`) — extract that table + `formatTokens`/`formatDuration`/`shortModel`/`totalTokens` into a shared `packages/workflows/src/tui/format.ts` and have BOTH `workflow-status.ts` and the reducer import it (no divergent formatting between the textual tool and the TUI; this is a pure move + re-export, no behavior change to the tool). The reducer holds NO clock and NO io — it is a fold over already-parsed lines, so a test feeds a hand-built `FeedEvent[]` and asserts the model.

**Files:**
- Create: `packages/workflows/src/tui/reducer.ts`, `packages/workflows/src/tui/format.ts`
- Modify: `packages/workflows/src/plugin/tools/workflow-status.ts` (import the moved helpers from `../../tui/format` instead of defining them locally)
- Test: `packages/workflows/src/tui/reducer.test.ts`, `packages/workflows/src/tui/format.test.ts`

**Verification:** `bun test packages/workflows/src/tui/` — RED-first: (1) `parseFeedLine` returns the typed event for each `FeedEvent` member and `undefined` for a truncated half-line / unknown `type` / non-JSON; (2) a hand-built feed (`run:start` → two phases of `agent:start`/`agent:launched`/`agent:stats`/`agent:end` + a cached end with no sessionID + `run:end`) reduces to the expected `phases`/`agents` with correct `done/total/marker`, durations from the enriched ends, and summed tokens; (3) concurrent same-label agents in one phase each retain their OWN sessionID-bound stats (the bug `workflow-status.ts:233-238` documents); (4) a `run:cancel-requested` before `run:end` yields top-level `status: "cancelling"` then the terminal status; (5) feeding only a prefix (no `run:end`) yields a coherent in-flight `running` state with open occurrences. `bun test packages/workflows/src/plugin/tools/workflow-status.test.ts` stays green after the helper move. `bun run typecheck` + `biome check packages/workflows/src/tui`.

**Done when:** an arbitrary feed file (full or mid-stream prefix) reduces to a phases/agents/stats model structurally equivalent to what `workflow_status` renders from the live handle, the formatting helpers live in one shared module imported by both, and the data layer is fully covered with zero opentui/solid/io dependency. Live TUI rendering is NOT exercised here (out of automated verification, per Epic scope).
...
**Implementation vision:** New module `packages/workflows/src/tui/tailer.ts`: `createFeedTailer({ path, onEvent, onError?, watchFn?, readFn?, statFn?, pollMs? })` → `{ start(): Promise<void>; stop(): void }`. On `start`: read the whole file from offset 0 (a missing file is not an error — emit nothing, begin watching the directory for the file's creation), split on `\n`, buffer any trailing partial line (no terminating newline) and DO NOT parse it until the next read completes the line — append-only writes guarantee a partial line is only ever a not-yet-flushed tail, never corruption. Each complete line goes through `parseFeedLine`; non-parseable lines are skipped (the reducer already tolerates `undefined`). Maintain a byte offset; on every change signal, `stat` the file and read only `[offset, size)`, advancing the offset. Change detection: `watchFn(path)` (defaulting to `node:fs`'s `watch`) drives reads; a `pollMs` interval (default 250ms) `stat`s and reads-on-growth as a FALLBACK for platforms/filesystems where `fs.watch` does not deliver events (network mounts, some container overlays) — both paths funnel through the same idempotent `readNew()` guarded by a single in-flight latch so a watch event and a poll tick never double-read the same bytes. `stop()` closes the watcher and clears the interval. All fs primitives are injectable (`watchFn`/`readFn`/`statFn`) so a test drives `readNew()` deterministically against an in-memory growing buffer with NO real timers or watchers. The tailer owns io ONLY; it never holds reducer state — the caller wires `onEvent` to the reducer's `apply`.

**Files:**
- Create: `packages/workflows/src/tui/tailer.ts`
- Test: `packages/workflows/src/tui/tailer.test.ts`

**Verification:** `bun test packages/workflows/src/tui/tailer.test.ts` — RED-first against injected fs: (1) a file written in three chunks where chunk 2 ends mid-line emits the buffered line only after chunk 3 completes it (no partial-line parse); (2) reading resumes from the byte offset — a second growth read parses ONLY the appended bytes, never re-emits earlier lines; (3) a missing file at `start` emits nothing and does not throw, then emits once the file appears; (4) watch-event and poll-tick arriving together produce each line exactly once (in-flight latch); (5) `stop()` halts further reads. `bun run typecheck` + `biome check packages/workflows/src/tui/tailer.ts`. NOTE: real `fs.watch` delivery is platform-dependent and is NOT asserted in CI — the poll fallback is the guaranteed path and the watch path is validated manually (Task 8.3.4 manual step).

**Done when:** a growing append-only feed file streams to `onEvent` line-by-line with no duplicate or partial-line emissions, a pre-existing completed file replays fully from offset 0, the watch+poll dual mechanism is deterministically unit-tested through injected fs, and `stop()` is clean.

#### Task 8.3.3: `./tui` entrypoint + full-screen route (Phases | Agents | Detail) with `j/k/enter/x/esc`

- [x] Done

**Context:** opencode resolves a TUI plugin's entry from `pkg.json.exports["./tui"]` (`.references/opencode/packages/opencode/src/plugin/shared.ts:104-106`, `kind = "tui"`) and `import()`s it in the unsandboxed TUI process. The workflows package today exports only `"."` (server plugin) and `"./lib"` (`packages/workflows/package.json:5-8`); it must gain `"./tui"`. The canonical full-screen-route plugin is `diff-viewer.tsx` (`.references/opencode/packages/opencode/src/cli/cmd/tui/feature-plugins/system/diff-viewer.tsx`): a `TuiPluginModule` `{ id, tui }` whose `tui(api)` calls `api.route.register([{ name, render: () => <View api={api}/> }])` (`diff-viewer.tsx:1026-1032`) and `api.keymap.registerLayer({ commands: [{ name, slashName, namespace:"palette", run(){ api.route.navigate(ROUTE, {...}) } }] })` for the open command (`diff-viewer.tsx:1034-1052`); in-route navigation uses `useBindings(() => ({ commands, bindings: [{key:"j,down",cmd:...}] }))` (`diff-viewer.tsx:716-729`) and the view is a `position:absolute` full-screen `<box>` (`diff-viewer.tsx:732`). Route/keymap types: `TuiRouteDefinition` (`.references/opencode/packages/plugin/src/tui.ts:69-72`), `TuiPluginApi` (`tui.ts:581-626`), `TuiPlugin`/`TuiPluginModule` (`tui.ts:628-633`). The type-lag gotcha (Phase 8 binding decision): the PUBLISHED `PluginModule.tui?: never` pins out `tui`, but the runtime accepts it; the TUI-specific `TuiPluginModule` from `@opencode-ai/plugin/tui` is the correct type and sidesteps the lag. opentui peers are declared in the plugin package (`.references/opencode/packages/plugin/package.json:21-33`: `@opentui/core`/`keymap`/`solid` `>=0.3.2`, all optional). The base tsconfig sets no `jsx` (`tsconfig.base.json` — every other package is `.ts`), while opencode's own tsconfig uses `jsx:"preserve"`+`jsxImportSource:"@opentui/solid"` (`.references/opencode/packages/opencode/tsconfig.json:5-6`); diff-viewer additionally carries a per-file `/** @jsxImportSource @opentui/solid */` pragma (`diff-viewer.tsx:1`).

**Implementation vision:** Wire the dual export and the route, keeping ALL pure logic in the 8.3.1/8.3.2 modules so the JSX stays thin (smoke-level testable only).
- **Package + build:** add `"./tui": "./src/tui/index.ts"` to `packages/workflows/package.json` exports beside `"."`/`"./lib"`; add `@opentui/core`/`@opentui/keymap`/`@opentui/solid` (pinned to the versions shipped with the pinned opencode — read `.references/opencode`'s lockfile/catalog and pin the EXACT version, not a range; record it in the module doc per the Phase 8 risk note) as `peerDependencies` + `devDependencies` (dev so typecheck resolves them locally), and `@opencode-ai/plugin` already at `1.16.2` provides `/tui`. Add a `packages/workflows/tsconfig.tui.json` extending the base with `"jsx":"preserve"`,`"jsxImportSource":"@opentui/solid"`,`"include":["src/tui"]`, and a per-file `/** @jsxImportSource @opentui/solid */` pragma on each `.tsx` (matching diff-viewer); extend the root `typecheck` script (`package.json`) to also run `bunx tsc --noEmit -p packages/workflows/tsconfig.tui.json`. Confirm `biome.json` lints `.tsx` (the `**` include already covers it; verify no rule trips on JSX).
- **Entry:** `packages/workflows/src/tui/index.ts` — `export default { id: "opencode-drawer-workflows", tui } satisfies TuiPluginModule` (typed against `@opencode-ai/plugin/tui`, NOT the server `PluginModule`). `tui(api)` resolves the feed dir (`<dataDir>/workflow-feed` via the same `resolveDataBaseDir` core helper the engine uses, `packages/core/src/persistence.ts` — the viewer reads, the engine writes, one resolution), registers the `workflows` route and a `palette`-namespace `workflows.open` command (`slashName: "workflows"`, an explicit keybind documented in the route).
- **Route view:** `packages/workflows/src/tui/route.tsx` — a `position:absolute` full-screen three-pane layout. The Phases pane lists `PhaseView`s with `marker name done/total`; the Agents pane lists the selected phase's `AgentView`s as CC-style rows (`marker label  model  <tokens> tok · <tools> tools · <duration>`, formatted via `src/tui/format.ts`); the Detail pane shows the selected agent's `lastTools`, `note`, full token breakdown, sessionID. State is Solid signals (`createSignal` selection indices) driven by ONE reducer per open feed file (8.3.1) fed by ONE tailer (8.3.2), started on mount, `stop()`ed on `onCleanup`. `useBindings`: `j/k` move selection within the focused pane, `enter` drills Phases→Agents→Detail (focus advance), `esc` backs out a pane then closes the route (navigate to the return route), `x` writes the cancel sentinel for the selected run — `mkdir(<dataDir>/workflow-control, {recursive})` + `writeFile(<dataDir>/workflow-control/<runId>.cancel, "")` via `node:fs/promises` (the EXACT external touch Task 8.2.3 proved end-to-end; the engine poll loop consumes it, the feed's `run:cancel-requested` line flips the view to `cancelling`). Which run the route shows: the open command navigates with `{ runId }` when one is known (from the sidebar, 8.3.4) or defaults to the most-recently-modified feed file (`stat` mtime over the feed dir). JSX is kept to layout + signal wiring; every derivation (rows, markers, totals) comes from the pure modules.

**Files:**
- Modify: `packages/workflows/package.json` (`"./tui"` export, opentui peer+dev deps), root `package.json` (typecheck script adds the tui tsconfig)
- Create: `packages/workflows/tsconfig.tui.json`, `packages/workflows/src/tui/index.ts`, `packages/workflows/src/tui/route.tsx`
- Test: `packages/workflows/src/tui/index.test.ts` (smoke: the default export is a `TuiPluginModule` with `id` + an async `tui`; calling `tui` against a fake `TuiPluginApi` that records calls registers exactly one route named `workflows` and one `palette` command `workflows.open`, and the `x` handler writes `<dir>/<runId>.cancel` through an injected fs)

**Verification:** `bun test packages/workflows/src/tui/index.test.ts` — RED-first: the module shape + route/command registration + sentinel-write handler are asserted against a hand-rolled fake `api` (no opentui runtime); the JSX render body is NOT mounted (live rendering is out of automated scope). `bun run typecheck` (now including `tsconfig.tui.json`) — the `.tsx` files type-check against `@opencode-ai/plugin/tui` + opentui peers with the jsx pragma, and the type-lag workaround is exercised by typing the export as `TuiPluginModule`. `bun run lint` covers `.tsx`. MANUAL validation step (live TUI, out of CI): with both surfaces installed and the pinned opencode, open the `workflows` route over a live run, confirm Phases/Agents/Detail update in real time from the feed, `j/k/enter/esc` navigate, and `x` cancels the run (record settles `cancelled`, view shows `cancelling` then terminal).

**Done when:** the `./tui` export loads as a `TuiPluginModule` under the pinned opencode, registers the `workflows` route + open command, the route renders the reducer's model and navigates with `j/k/enter/esc`, `x` writes the 8.2 sentinel, the pinned opentui versions + type-lag workaround are documented in the module, and all automated gates (the smoke test, typecheck incl. the tui tsconfig, lint) are green. The live-render behavior is validated by the documented manual step.
...
**Implementation vision:** Extend `packages/workflows/src/tui/index.ts`'s `tui(api)` to also `api.slots.register({ order, slots: { sidebar_content(_ctx, _props) { return <SidebarRuns api={api} feedDir={feedDir}/> } } })`. New `packages/workflows/src/tui/sidebar.tsx`: a small pure-driven view that periodically lists the feed dir, runs each file's tail through a lightweight summary (reuse the reducer; for the sidebar only the top-level `status` + aggregate `done/total` across phases + `startedAt` elapsed are needed — expose a `summarize(state): { runId; activeAgents; totalAgents; elapsedMs; status }` helper in `reducer.ts`), filters to non-terminal runs, and renders one line per active run `marker <short-runId> <done>/<total> agents · <elapsed>` using `src/tui/format.ts`. `<Show when={active().length > 0}>` so the slot is invisible with no live runs (todo-plugin idiom). The directory scan is a polled `createMemo`/`createResource` on a coarse interval (e.g. 1s — the sidebar is a glance, not a live ticker); the heavy live updating belongs to the route's tailer, not the sidebar. Clicking/selecting a run (or the documented open keybind from 8.3.3) navigates `api.route.navigate("workflows", { runId })`. The elapsed uses the same `formatDuration` as everything else. This task ALSO carries the Epic's manual end-to-end walkthrough (the live-render proof that is out of automated CI): documented steps to install both surfaces against the pinned opencode, launch a multi-phase workflow, watch the sidebar summarize it, open the route, see Phases/Agents/Detail update live, press `x` to cancel through the sentinel, and restart the TUI to confirm the settled feed file still renders.

**Files:**
- Modify: `packages/workflows/src/tui/index.ts` (register `sidebar_content`), `packages/workflows/src/tui/reducer.ts` (add `summarize`)
- Create: `packages/workflows/src/tui/sidebar.tsx`
- Modify: `packages/workflows/README.md` (or the authoring guide) — the "install both surfaces + live walkthrough" manual validation steps and the pinned-opencode/opentui version note
- Test: `packages/workflows/src/tui/reducer.test.ts` (extend: `summarize` over a multi-phase in-flight state yields the expected `activeAgents`/`totalAgents`/`elapsedMs`/`status`; a settled state summarizes with the terminal status)

**Verification:** `bun test packages/workflows/src/tui/reducer.test.ts` — RED-first: `summarize` aggregates phase counts and elapsed correctly for in-flight and settled states. `bun run typecheck` (incl. `tsconfig.tui.json`) — `sidebar.tsx` type-checks against the slot signature. `bun run lint`. MANUAL validation (the Epic-level live proof, out of CI, documented in the README walkthrough): with both surfaces installed under the pinned opencode, a running multi-phase workflow appears in the sidebar as `34/35 agents · 1h34m`-style, opening the route shows live Phases/Agents/Detail, `x` cancels via the sentinel, and a TUI restart re-renders the now-settled run from its feed file.

**Done when:** the `sidebar_content` slot lists active runs (invisible when none), navigates into the `workflows` route, `summarize` is unit-covered, the pinned-version note and the full manual walkthrough are documented, and the full automated suite + typecheck (incl. the tui tsconfig) + lint are green. The Epic's live-render Done-when is satisfied by the executed manual walkthrough.
