...
| 8 | Live observability + native viewer: real-time per-run feed with per-agent tokens/tool-calls/duration, external cancel channel, and a `./tui`-surface full-screen viewer (Phases \| Agents \| Detail) mirroring CC's `/workflows` overlay | 8.1, 8.2, 8.3 | 8.1–8.3 Detailed |
